...
const UserProfile = () => {
  ...
  const { orders: cardOrders } = useSelector(
    (state) => state.cardMyOrderNotPaidList,
  )
  const { orders: cocOrders } = useSelector(
    (state) => state.changeOfCourseOrderListMy,
  )
  const { orders: olruOrders } = useSelector(
    (state) => state.oLevelUploadOrderListMy,
  )
  const { orders: jprOrders } = useSelector(
    (state) => state.jambPasswordResetOrderListMy,
  )
  const { loading: makeAdminLoading } = useSelector(
    (state) => state.userMakeAdmin,
  )
  const { loading: makeEditorLoading } = useSelector(
    (state) => state.userMakeEditor,
  )
  ...
  const handleAddAmountToUser = () => {
    dispatch(adminCreditWallet(userId, amount))
  }
  ...
  return (
    <UserProfileContainer>
      {loading ? (
            ...
            <CardContent>
              ...
              <div className="contents">
                ...
                <div>
                  ...
                  <span style={{ color: user.confirmed ? 'green' : 'red' }}>
                    {user.confirmed ? 'confirmed' : 'Not confirmed'}
                  </span>
                </div>
                <div>
                  <span>Admin status:</span>
                  <span>{user.admin ? 'Admin' : 'Not Admin'}</span>
                </div>
                <div>
                  <span>Sign up with:</span>
                  <span>
                    {user.type.replace(/\b\w/g, (l) => l.toUpperCase())}
                  </span>
                </div>
                <div>
              ...
              <div className="actions">
                {userInfo.isAdmin && (
                  <>
                    <FormControlLabel
                      control={
                        <Switch checked={isAdmin} onChange={handleMakeAdmin} />
                      }
          ...
          <MessageModal
            ...
            message={
              <div className="delete">
                ...
                {user.wallet > 0 && (
                  <p>
                    This user has funds in their wallet. On selection "Yes", the
                    wallet will also be cleared.
                  </p>
                )}
        ...
        <Card>
          <CardContent>You have not selected any user</CardContent>
        </Card>
      )}
    </UserProfileContainer>
  )
